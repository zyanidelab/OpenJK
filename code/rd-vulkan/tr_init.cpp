...
void R_TakeScreenshot( int x, int y, int width, int height, char *fileName ) {
	...
	qboolean bSaveAsJPG = (qboolean)!Q_stricmpn(&fileName[strlen(fileName)-4],".jpg",4);

	if (bSaveAsJPG)
	{
		// JPG saver expects to be fed RGBA data, though it presumably ignores 'A'...
		//
		buffer = (unsigned char *) ri.Malloc(glConfig.vidWidth*glConfig.vidHeight*4, TAG_TEMP_WORKSPACE, qfalse, 4);
		...
		RE_SaveJPG(fileName, 95, width, height, buffer, 0); //TODO: Check padding thing
	}
	else
	{
		// TGA...
		//
		buffer = (unsigned char *) ri.Malloc(glConfig.vidWidth*glConfig.vidHeight*3 + 18, TAG_TEMP_WORKSPACE, qfalse, 4);
		byte *pixbuffer = (unsigned char *) ri.Malloc(glConfig.vidWidth*glConfig.vidHeight*4, TAG_TEMP_WORKSPACE, qfalse, 4);
		memset (buffer, 0, 18);
...
static void R_LevelShot( void ) {
	...
	source = (byte*) ri.Malloc( glConfig.vidWidth * glConfig.vidHeight * 3, TAG_TEMP_WORKSPACE, qfalse, 4 );

	buffer = (byte*) ri.Malloc( LEVELSHOTSIZE * LEVELSHOTSIZE*3 + 18, TAG_TEMP_WORKSPACE, qfalse, 4 );
	memset (buffer, 0, 18);
	buffer[2] = 2;		// uncompressed type
    ...
    byte* buffer2 = //(byte*) ri.Hunk_AllocateTempMemory(glConfig.vidWidth*glConfig.vidHeight*4);
                    (byte*) R_Malloc(glConfig.vidWidth*glConfig.vidHeight*4, TAG_TEMP_WORKSPACE, qfalse);
