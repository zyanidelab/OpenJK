...
void RE_StretchRaw (int x, int y, int w, int h, int cols, int rows, const byte *data, int iClient, qboolean bDirty )
{
	...
	if ( cols != tr.scratchImage[iClient]->width || rows != tr.scratchImage[iClient]->height )
	{
		...
		/*qglTexImage2D( GL_TEXTURE_2D, 0, GL_RGB8, cols, rows, 0, GL_UNSIGNED_BYTE, data );
	...
	else
	{
		if (bDirty)	// FIXME: some TA addition or other, not sure why, yet. Should probably be true 99% of the time?
		{
			...
			//qglTexSubImage2D( GL_TEXTURE_2D, 0, 0, 0, cols, rows, GL_UNSIGNED_BYTE, data );
			const Vk_Image& image = vk_world.images[tr.scratchImage[iClient]->index];
			vk_upload_image_data(image.handle, cols, rows, false, data, 4);
	#ifdef TIMEBIND
			if ( r_ignore->integer )
			{
    ...
    tr.cinematicShader->stages[0].bundle[0].image = tr.scratchImage[iClient];
...
void RE_GetScreenShot(byte *buffer, int w, int h)
{
	...
	source = (byte *)R_Malloc(glConfig.vidWidth * glConfig.vidHeight * 4, TAG_TEMP_WORKSPACE, qfalse);
	memcount = glConfig.vidWidth * glConfig.vidHeight * 4;

	// gamma correct
	/*if(glConfig.deviceSupportsGamma)
		R_GammaCorrect(source, memcount);*/
...
//	ri.Printf( PRINT_ALL, "RE_InitDissolve()\n");
		...
		)
	{
		...
		if (pBuffer)
		{
			...
			//
			//qglReadPixels (0, 0, glConfig.vidWidth, glConfig.vidHeight, GL_UNSIGNED_BYTE, pBuffer );
